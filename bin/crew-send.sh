...
#        crew-send.sh <id> --re-ring <record-path>     (the watcher's ladder)
#
# The durable record is the delivery. The terminal receives one short
# self-describing doorbell line, so a swallowed or duplicated ring is harmless:
# the crew finds the inbox empty or already handled. Delivery is proved by the
# crew moving the record into handled/, never by the Enter key.
set -eu
...
if [ "${1:-}" = --re-ring ]; then
  REC=${2:-}
  [ -n "$REC" ] && [ -f "$REC" ] || foreman_die "--re-ring needs an existing record path"
  GOTO=1
else
  REC=
  GOTO=0
...
if [ "$GOTO" -eq 0 ]; then
  ...
  trap - EXIT
  printf '%s\t0\t%s\n' "$(basename "$REC")" "$(date +%s)" >"$DIR/inbox/.ring"
fi

PANE=$(foreman_pane_of "$ID" 2>/dev/null || true)
RANG=no
if [ -n "$PANE" ]; then
  # Leading ": " is a shell no-op, so this line does nothing if the crew's agent
  # has exited into a bare shell.
